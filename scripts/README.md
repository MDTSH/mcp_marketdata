...
| 文件 | 作用 |
|---|---|
| `prepare_sync.py` | 从 `Z:\market_data\snapshots` 复制 90 自然日窗口到 `../snapshots`，裁剪 HIST 行，写 `../reports/SYNC_REPORT_YYYYMMDD.md`。不执行 git push。 |
| `verify_window.py` | 断言目标 `snapshots/` 无窗口外 JSON，且所有 `hist_file` 存在。 |
